...
export default function (babel) {
  ...
  function resolveToRootBinding(name, scope) {
    let currentName = name;
    let lastBinding = scope.getBinding(name);
    ...
    while (currentName && !visited.has(currentName)) {
      ...
      lastBinding = binding; // Guardamos el binding actual como el "último conocido"

      const node = binding.path.node;

      // Si es un alias (var a = b), seguimos la cadena
      if (t.isVariableDeclarator(node) && t.isIdentifier(node.init)) {
        currentName = node.init.name;
      } else {
        // Hemos llegado a la definición real (una función, un literal, etc.)
        break;
      }
    }
